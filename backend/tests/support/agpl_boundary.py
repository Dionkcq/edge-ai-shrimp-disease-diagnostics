...
`model/training/` and `model/pipeline/` use Ultralytics/PyTorch, which are AGPL-3.0.
The served application must never reach them. This module implements the checks;
`tests/security/test_dependency_boundary.py` is what runs them.
...
RUNTIME_ENTRY_MODULE = "shrimp_server.main"
