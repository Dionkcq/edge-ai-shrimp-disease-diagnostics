...
def test_the_repository_ships_no_signed_acceptance_file() -> None:
    # model/pipeline/tests/test_pipeline.py -> ../../../
    root = Path(__file__).resolve().parents[3]
    assert not (root / "data" / "mapping_acceptance.json").exists(), (
        "data/mapping_acceptance.json must not exist in the repository: signing it "
        "is a reviewer's decision, not a checked-in default"
    )
    assert (root / "data" / "mapping_acceptance.example.json").is_file()
